...
export default function Projects() {
  ...
  filtered.filter(p => p.parent_id).forEach(p => {
    if (!subProjectsByParent[p.parent_id]) subProjectsByParent[p.parent_id] = [];
    subProjectsByParent[p.parent_id].push(p);
  });

  // When searching, auto-expand groups that have matching sub-projects
  const effectiveExpanded = { ...expandedGroups };
  if (search) {
    Object.keys(subProjectsByParent).forEach(parentId => {
      effectiveExpanded[parentId] = true;
    });
  }
    ...
    <div>
            ...
            return (
              <div key={project.id}>
                ...
                >
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); if (subs.length > 0) toggleGroup(project.id); }}
                    className={`w-5 h-5 flex items-center justify-center flex-shrink-0 ${subs.length > 0 ? 'text-gray-400 hover:text-gray-600' : 'text-transparent cursor-default'}`}
                  >
                    {subs.length > 0 && (
                  ...
                  {/* Status dot */}
                  <span
                    className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${project.status === 'active' ? 'bg-green-500' : 'bg-gray-300'}`}
                    title={statusLabels[project.status] || project.status}
                  />

                  {/* Name + Sub-project count badge */}
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400 flex-shrink-0">
                    ...
                    <span className="flex items-center gap-1" title="Test Scripts">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                {/* Sub-projects (when expanded) */}
                {isExpanded && subs.map((sub) => (
                  <div
                    key={sub.id}
                    onClick={() => navigate(`/projects/${sub.id}`)}
                    className="flex items-center gap-4 pl-14 pr-4 py-2.5 hover:bg-blue-50/50 transition-colors cursor-pointer group border-t border-gray-50"
                  >
                    <span className="text-gray-300 flex-shrink-0">&mdash;</span>
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${sub.status === 'active' ? 'bg-green-500' : 'bg-gray-300'}`}
                    />
                    <span className="text-sm text-gray-700 min-w-0 truncate flex-1">{sub.name}</span>
                    <span className={`text-xs font-medium flex-shrink-0 ${priorityColors[sub.priority] || 'text-gray-400'}`}>
                      {priorityLabels[sub.priority] || 'Medium'}
                    </span>
                    <div className="flex items-center gap-4 text-xs text-gray-400 flex-shrink-0">
                      <span className="flex items-center gap-1" title="Members">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                      ...
                      <span className="flex items-center gap-1" title="Test Scripts">
                        ...
                        {sub.test_script_count}
                      </span>
                    </div>
                    <div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link
                        to={`/projects/${sub.id}/edit`}
                        onClick={(e) => e.stopPropagation()}
                        className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
                        title="Edit project"
                      >
                      ...
                      <OverflowMenu
                        ...
                        hasTestScripts={sub.test_script_count > 0}
                      />
                    </div>
                  </div>
                ))}
      ...
      {!loading && filtered.length > 0 && (
        <p className="mt-4 text-sm text-gray-500">
          Showing {mainProjects.length} project{mainProjects.length !== 1 ? 's' : ''}
          {filtered.filter(p => p.parent_id).length > 0 && ` with ${filtered.filter(p => p.parent_id).length} sub-project${filtered.filter(p => p.parent_id).length !== 1 ? 's' : ''}`}
        </p>
      )}
    </div>
  );
